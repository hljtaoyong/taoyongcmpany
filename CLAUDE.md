# Tao Yong Company - 现代化 Web 应用构建框架
Vite 6 + React 19 + TailwindCSS v4 + shadcn/ui + react-router + Framer Motion + Supabase

**设计系统核心原则**：一切设计必须来自设计系统的颜色和组件
- 所有色彩派生自 CSS 变量 + color-mix
- 所有组件融合渐变 + 立体阴影 (微拟物语言)
- 禁止硬编码颜色值，禁止 backdrop-blur

**Apple 级动效系统**：Spring 物理引擎驱动
- 所有动画优先使用 Spring > 线性缓动
- 阻尼值 25-40，刚度 150-500
- Button: whileTap 回弹 (scale: 0.96)
- Card: whileHover 提升 (y: -4)
- 页面路由: AnimatePresence 丝滑过渡

**认证系统**：Supabase + Google OAuth
- @/lib/supabase.js: Supabase 客户端初始化
- @/lib/auth.js: 认证工具函数
- @/contexts/AuthContext.jsx: 全局认证状态
...
vite.config.js - 构建配置与插件
jsconfig.json - 路径别名 (@/* → src/*)
tailwind.config.cjs - TailwindCSS 配置,shadcn/ui 兼容
components.json - shadcn/ui 组件配置
index.html - HTML 入口模板
vercel.json - Vercel 部署配置 (SPA 路由重写/缓存/安全头)
.env - 环境变量 (Supabase 凭证)
.env.example - 环境变量示例
</config>

<routes>
/ - Landing Page (完整落地页,8 sections 转化漏斗)
/design-system - 设计系统展示页
/todos - todos 应用页 (增删改查/筛选统计)
</routes>
