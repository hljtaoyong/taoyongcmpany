 ...
 * [OUTPUT]: 导出 AlarmsPage 完整页面,闹钟列表/增删改查/浏览器通知,苹果极简风格
...
function AlarmItem({ alarm, onToggle, onEdit, onDelete, onComplete }) {
  ...
  return (
    <motion.div
      ...
      className={`relative overflow-hidden bg-white rounded-xl p-5 border transition-all ${
        alarm.is_completed ? 'opacity-60 border-gray-200' : 'border-gray-200'
    ...
    >
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <h3 className={`font-semibold text-lg ${alarm.is_completed ? 'line-through text-gray-400' : 'text-gray-900'}`}>
            {alarm.title}
          </h3>
          {alarm.description && (
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-5xl mx-auto px-8 py-8">
        ...
        >
          <motion.div
            variants={staggerItem}
            className="bg-blue-600 rounded-xl p-6 text-white shadow-sm"
          >
          ...
          <motion.div
            variants={staggerItem}
            className="bg-white rounded-xl p-6 text-gray-700 border border-gray-200 shadow-sm"
          >
        ...
        {error && (
          <motion.div
            initial={{ opacity: 0, y: -10 }}
            animate={{ opacity: 1, y: 0 }}
            className="bg-red-50 border border-red-200 text-red-600 px-4 py-3 rounded-lg mt-6"
          >
            {error}
          </motion.div>
        )}
